...
class GreekValidator:
    ...
    FIXED: Added token update lock to prevent race conditions during token refresh.
    """
    def __init__(self, validated_cache: Dict[str, dict], sabr_model: EnhancedSABRModel, refresh_sec: int = 15, tolerance_pct: float = 15.0):
        ...
        # CRITICAL FIX: Token Update Lock to prevent race conditions
    ...
    async def update_token(self, new_token: str):
        """
        Thread-safe token update method.
        Call this from API routes when refreshing tokens.
        """
        async with self._token_lock:
            self.token = new_token
            logger.info("✅ Greek Validator token updated successfully")
    ...
    async def _fetch_broker_greeks(self) -> Dict[str, dict]:
        """
        FIXED: Uses token lock to prevent race conditions during fetch.
        """
        chunk_size = 500
        ...
        # Acquire token lock for consistent headers during fetch
        async with self._token_lock:
            headers = {
