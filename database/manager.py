...
class HybridDatabaseManager:
    """ACID-compliant storage with advanced analytics"""
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute('''INSERT INTO trades 
                (strategy_type, status, entry_time, lots, net_premium, expiry_date, 
                 trade_vega, trade_delta, max_loss, pnl) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''', 
                (trade.strategy_type, trade.status.value, trade.entry_time.isoformat(),
                 ...
                 trade.trade_vega, trade.trade_delta, trade.max_loss_per_lot, 0.0))
            return cursor.lastrowid
    
    def update_trade_close(self, trade_id: int, pnl: float, reason: str):
        with sqlite3.connect(self.db_path) as conn:
            conn.execute('''UPDATE trades SET status=?, pnl=?, exit_reason=?, exit_time=? WHERE id=?''',
    ...
    def save_portfolio_snapshot(self, metrics: PortfolioMetrics):
        with sqlite3.connect(self.db_path) as conn:
            conn.execute('''INSERT INTO portfolio_snapshots 
                (timestamp, total_pnl, total_delta, total_gamma, total_theta, total_vega, open_trades)
                VALUES (?, ?, ?, ?, ?, ?, ?)''',
                (metrics.timestamp.isoformat(), metrics.total_pnl, metrics.total_delta,
                 metrics.total_gamma, metrics.total_theta, metrics.total_vega, metrics.open_trades))
    
    def save_market_analytics(self, metrics: AdvancedMetrics):
        with sqlite3.connect(self.db_path) as conn:
            conn.execute('''INSERT INTO market_analytics 
                (timestamp, spot_price, vix, ivp, regime, event_risk)
                VALUES (?, ?, ?, ?, ?, ?)''',
                (metrics.timestamp.isoformat(), metrics.spot_price, metrics.vix,
                 metrics.ivp, metrics.regime.value, metrics.event_risk_score))
    ...
    def save_daily_state(self, daily_pnl: float, max_equity: float, cycle_count: int, total_trades: int):
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.execute('''INSERT OR REPLACE INTO daily_state 
                (date, daily_pnl, max_equity, cycle_count, total_trades)
                VALUES (?, ?, ?, ?, ?)''', 
