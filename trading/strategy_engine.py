...
class IntelligentStrategyEngine:
    ...
    def select_strategy_with_capital(self, metrics: AdvancedMetrics, spot: float,
        ...
        if metrics.regime == "BINARY_EVENT":
            logger.warning(f"☢ BINARY EVENT ({metrics.top_event}).")
        ...
        if is_expiry_week:
            ...
            if is_rich and is_backwardation:
    ...
    def _generate_pro_legs(self, strategy, spot, expiry, metrics) -> List[Dict]:
        ...
        try:
            ...
            # 🔧 FIX: Removed BankNifty logic. Pure NIFTY rounding.
            width = max(100, self._round_strike(implied_move))
            strangle_width = max(150, self._round_strike(implied_move * 1.5))
            ...
            # BROKEN WING BUTTERFLY (Ratio Spread Fix)
                ...
                # Capped Wing Distance (Max 1500pts for NIFTY)
                max_wing = 1500
                wing_dist = min(max_wing, implied_move * 2.5)
    ...
    # ============================================
    # 🔧 FIX: Strike rounding strictly for NIFTY (50pts)
    # ============================================
    def _round_strike(self, price):
        """
        Round strike to valid NIFTY 50 intervals (50 points).
        """
        interval = 50 
        return round(price / interval) * interval

    def _get_far_expiry(self, near_expiry_str):
        if not self.instruments_master:
            return None
        ...
        # First try: Find monthly expiry (25-45 days out)
        for e in all_exp:
            days = (e - near_dt).days
            if 25 <= days <= 45:
                return e.strftime("%Y-%m-%d")
        
        # Fallback: Any expiry 7-60 days out
        for e in all_exp:
            days = (e - near_dt).days
            if 7 < days <= 60:
                return e.strftime("%Y-%m-%d")
