#!/usr/bin/env python3
"""
VolGuard 20.0 Fortress Engine (V3 Hybrid) 24x7 Market-Aware Edition
...
MARKET-AWARE: WebSocket only when NSE is open; historical fetch always active.
"""
...
class VolGuard20Engine:
    ...
    async def initialize(self) -> None:
        ...
        # --- FIXED: FORCE LOAD HISTORICAL DATA ---
        # Old logic skipped loading if today's EOD wasn't ready. 
        # New logic loads whatever exists in DB immediately.
    ...
    async def _trading_logic(self, spot: float) -> None:
        ...
        try:
            ...
            self.running = False
            # Try to alert sheriff
            try:
                if hasattr(self, 'alerts') and self.alerts:
                    await self.alerts.send_critical_alert(
                        f"ROLLBACK FAILURE {trade.id}\nSystem halted. Manual position closure required."
                    )
            except: pass
            
            raise EngineCircuitBreaker("Rollback failure - manual intervention required") from e
    ...
    async def _adopt_zombie(self, token: str, qty: int) -> None:
        ...
        try:
            ...
            logger.warning(f"⚠️ Zombie {token} fallback price {price:.2f}")
        ...
        trade = MultiLegTrade(
            id=f"ZOMBIE-{int(time.time()*1000)}",
            legs=[dummy],
