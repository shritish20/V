...
class VolGuard17Engine:
    def __init__(self):
        ...
        # CRITICAL UPDATE: Passing DB Manager to Capital Allocator
    ...
    async def _consider_new_trade(self, spot: float):
        ...
        # 3. Real-Time Chain Metrics (NEW: Get Skew and Straddle Price)
        expiry = self.strategy_engine._get_expiry_date(ExpiryType.WEEKLY)
        pcr = 1.0
        skew = 0.0
        straddle_price = spot * 0.01 # Fallback
        ...
        # 4. Build Metrics (QUANT UPGRADE: Wired new fields)
    ...
    async def _execute_new_strategy(self, strat_name, legs_spec, exp_type, bucket):
        ...
        try:
            ...
            if success:
                ...
                # NEW: TRIGGER AI CIO
                # This generates the 'Why' explanation
                try:
                    bias_str = "BULLISH" if self.last_metrics.pcr > 1.25 else \
                               "BEARISH" if self.last_metrics.pcr < 0.7 else "NEUTRAL"
            ...
            if settings.GREEK_VALIDATION:
                ...
                if broker_greeks:
                    ...
                    greeks = GreeksSnapshot(
                        ...
                        theta=bg.get("theta", 0), vega=bg.get("vega", 0), iv=bg.get("iv", 0)
                    )
        except Exception:
            pass
    ...
    def get_system_health(self):
        # Note: Capital allocator status is now async, so we return a placeholder here
        # The API endpoint /health calls this, but we'll need to update that to await if possible,
        # or just return basic info here. For now, we omit the async call to avoid breaking sync code.
        return {
    ...
    async def get_dashboard_data(self):
        # Made async to support DB call for capital status
