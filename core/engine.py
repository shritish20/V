...
class VolGuard17Engine:
    ...
    async def _consider_new_trade(self, spot: float):
        """
        The Main Brain Loop - Now Wired with Real Data
        """
        # 1. Market Data
        vix = self.rt_quotes.get(settings.MARKET_KEY_VIX, 15.0)
        
        # 2. Estimate Daily Return (Trend)
        prev_close = 0.0
        ...
        # 5. Fetch Real-Time Option Chain for PCR & Skew
    ...
    async def _run_sabr_calibration(self):
        if not self._calibration_semaphore.locked():
            ...
            logger.debug("SABR calibration already running, skipping...")
