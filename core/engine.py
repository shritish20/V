...
class VolGuard17Engine:
    def __init__(self):
        ...
        # --- CRITICAL INJECTION FIX ---
        # We pass self.pricing to the strategy engine so it can calculate Deltas
    ...
    async def initialize(self):
        """
        Startup Logic: 
        Instruments -> DB -> Restore -> Subscriptions -> Feed Start
        """
        ...
        # 4. Subscribe to all restored/zombie instruments BEFORE starting feed
        initial_tokens = set()
    ...
    async def _run_sabr_calibration(self):
        """
        Fetches data and runs calibration in non-blocking Executor.
        Includes Explicit Fallback if calibration fails.
        """
        ...
        try:
            ...
            if success:
                ...
                logger.warning("⚠️ SABR Calibration returned False. Reverting to fallback defaults.")
                self.sabr.reset() # Uses default Alpha=0.2, etc.
